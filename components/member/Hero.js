 ...
 * 背景に castle 紋章（完全形フルクレスト）を薄いウォーターマークとして敷き、
 * 前面に QUALIA 箔ワードマーク（Cinzel・gold-clip）＋アクロスティック（金clip頭文字＋ヘアライン短罫）
 * ＋主役（castle講師）丸写真72px（金ヘアラインリング）＋「メニューへ↓」。
 * 紋章は途切れさせない（背景に完全形で内包）方針。
 */
const ACROSTIC = [
  { c: 'Q', w: 'Quality' },
  { c: 'U', w: 'Unique' },
...
export default function Hero({ lead }) {
  return (
    <section className="relative bg-gradient-to-b from-navy-900 via-navy-700 to-navy-500 px-5 pt-20 pb-16 md:px-10 text-center overflow-hidden">
      ...
      {/* castle 紋章ウォーターマーク（完全形フルクレスト・背景／ロゴを途切れさせない） */}
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img
        src="/brand/castle-crest-white.png"
        alt=""
        aria-hidden="true"
        className="pointer-events-none select-none absolute left-1/2 top-12 -translate-x-1/2 w-[320px] max-w-[84%] h-auto opacity-[0.12]"
      />

      {/* 前面コンテンツ（ウォーターマークの上に重ねる） */}
      <div className="relative z-10">
        ...
        {lead && (
          <div className="mt-12 flex flex-col items-center">
            ...
            >
              {(lead.hero_photo_url || lead.photo_url) ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img
                  src={lead.hero_photo_url || lead.photo_url}
                  alt={lead.name}
