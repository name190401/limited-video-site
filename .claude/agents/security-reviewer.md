...
# 進め方
0. **実ファイル突合の鉄則**：spec・提案・実装が「このファイル/関数/API/コンポーネントを使う・流用する・参照する」と主張していたら、**判定前に必ずその実ファイルを開いて内容と突合する**。名前や申告を鵜呑みにしない。例：`app/api/password/route.js` を「viewer の合言葉照合に流用」と書いてあっても、実際は管理者用エンドポイントかもしれない／`lib/auth/layer2.js` が既に正規バックエンドとして存在するのに spec が localStorage を指定しているかもしれない。**参照先と実体が食い違っていたら ✗**。
