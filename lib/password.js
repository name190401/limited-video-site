...
/**
 * 日付ベースでパスワードを生成
 * シークレットキー + 日付 + グループインデックス → SHA256ハッシュ → 6文字の英数字
 */
export function generateDailyPassword(secretKey, date, groupIndex = 0) {
  const dateStr = date.toISOString().split('T')[0]; // YYYY-MM-DD
  const combined = secretKey + dateStr + String(groupIndex);
...
export function getTodayPassword(groupIndex = 0) {
  ...
  return generateDailyPassword(secretKey, new Date(), groupIndex);
}

/**
 * 指定日数分のパスワードを取得（全グループ対応）
 */
export function getPasswordsForDays(days = 7, groupCount = 1) {
  ...
  for (let i = 0; i < days; i++) {
    ...
    for (let g = 0; g < groupCount; g++) {
      groups.push({
        groupIndex: g,
        password: generateDailyPassword(secretKey, date, g),
      });
    }
