 ...
 *
 * `lib/auth/session-version.js` の `readPasswordVersion`（照合用・フェイルオープン）と
 * 必ず使い分ける。**Cookie を発行する経路と世代を +1 する経路はこちら**を使うこと：
 * あちらは読めないと 0 を返すため、発行時に使うと発行直後の Cookie が即失効し、
 * 更新時に使うと世代を巻き戻して失効済みセッションを復活させてしまう。
 * @param {string} key
