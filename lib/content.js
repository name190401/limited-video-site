 ...
 *
 * 重要原則: layer2（プラン）の youtube_id は、Layer2 トークン検証を通った
 * /api/plan/content からの getPlanVideos() でしか返さない。
...
export async function getSectionVideos(sectionKey) {
  ...
  const { data } = await supabase
    ...
    .order('sort_order');
  return (data || []).map((v) => {
    if (v.protection === 'layer2') {
      const { youtube_id, ...rest } = v;
      return { ...rest, locked: true, youtube_id: null };
    }
  ...
  for (const v of data || []) {
    const safe =
      v.protection === 'layer2'
        ...
        : { ...v, locked: false };
    (grouped[v.section_key] ||= []).push(safe);
  }
  return grouped;
}

/**
 * プラン（layer2）動画の実 ID を返す。
 * **必ず Layer2 トークン検証後（/api/plan/content）からのみ呼ぶこと。**
 */
export async function getPlanVideos() {
  if (isLocalMode()) return localPlanVideos();
  const supabase = getServiceClient();
  const { data } = await supabase
    .from('videos')
    .select('id, title, variant, audio_muted, youtube_id, status, sort_order')
    .eq('section_key', 'plan')
